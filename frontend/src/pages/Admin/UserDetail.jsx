    ...
    <AdminLayout>
      ...
      <div className="bg-[#111116] border border-white/[0.08] rounded-2xl p-6 mb-6">
        <div className="flex flex-col sm:flex-row items-start gap-5">
          <div className="w-16 h-16 rounded-2xl bg-white/[0.08] border border-white/[0.1] flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">
            {user.profilePhoto ? (
              <img src={user.profilePhoto} alt={user.userFullName} className="w-full h-full rounded-2xl object-cover" />
            ) : (
              user.userFullName?.charAt(0)?.toUpperCase() || "U"
            )}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-white mb-1">{user.userFullName}</h1>
            <p className="text-sm text-gray-400">{user.preferredStyle || "CLASSIC BESPOKE CLIENT"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-[#111116] border border-white/[0.08] rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: "Phone", value: user.userMobileNumber || "—" },
              { label: "Delivery Address", value: user.deliveryAddress || "—" },
            ].map((item) => (
        ...
        <div className="bg-[#111116] border border-white/[0.08] rounded-2xl p-6">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-xs text-gray-500 uppercase tracking-wider">Body Notes</span>
              <p className="text-sm text-gray-300 mt-1">{user.bodyNotes || "No body notes"}</p>
            </div>
      ...
      {/* Joined Date */}
      <div className="mb-6">
        <p className="text-xs text-gray-500">Joined: {new Date(user.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
      </div>
              ...
              {orders.length === 0 ? (
                ...
                orders.map((o) => (
                  <tr key={o._id} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors cursor-pointer" onClick={() => navigate(`/admin/orders/${o._id}`)}>
                    ...
                    <td className="px-6 py-3.5 text-sm text-white">{o.tailor?.tailorName || o.tailorName || "—"}</td>
